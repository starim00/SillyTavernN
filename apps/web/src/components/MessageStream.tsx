...
function frameOverflowAdaptationScript(): string {
  return `<style>[data-stn-clip-incidental-overflow-x="true"]{overflow-x:clip!important}</style><script>(()=>{const root=document.body;if(!root)return;const attribute="data-stn-clip-incidental-overflow-x";let queued=false;const sync=()=>{queued=false;const elements=[root,...root.querySelectorAll("*")];for(const element of elements)element.removeAttribute(attribute);for(const element of elements){const style=getComputedStyle(element);if(style.overflowX==="auto"&&(style.overflowY==="auto"||style.overflowY==="scroll")&&element.scrollHeight>element.clientHeight+1&&element.scrollWidth<=element.clientWidth+1)element.setAttribute(attribute,"true")}};const schedule=()=>{if(queued)return;queued=true;requestAnimationFrame(sync)};schedule();if(typeof ResizeObserver!=="undefined")new ResizeObserver(schedule).observe(root);new MutationObserver(schedule).observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:["class","style","hidden"]});addEventListener("resize",schedule)})()</script>`;
}
...
<html>
  ...
  <body>${compatibilityScript}${displayContent}${frameDialogueDecorationScript()}${frameOverflowAdaptationScript()}</body>
</html>`;
}
